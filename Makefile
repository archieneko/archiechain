...
build:
	...
	go build -o archie -ldflags="\
    	-X 'github.com/archieneko/archiechain/versioning.Version=$(LATEST_VERSION)' \
		-X 'github.com/archieneko/archiechain/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/archieneko/archiechain/versioning.Branch=$(BRANCH)'\
		-X 'github.com/archieneko/archiechain/versioning.BuildTime=$(TIME)'" \
	main.go
...
test-e2e:
	...
	go build -race -o artifacts/archie .
	env EDGE_BINARY=${PWD}/artifacts/archie go test -v -timeout=30m ./e2e/...
